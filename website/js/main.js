...
;(function () {
	...
	async function listOrders() {
		...
		const { data } = await axios.get('https://onchain.sv/api/v1/events?app=run.relayx.io&type=order')


		return data.events.map(event => event).filter(event => !!event).sort((a, b) => {
			return a.content.props.no > b.content.props.no ? 1 : -1
		})
	...
	async function listRankings() {
		...
		const { data } = await axios.get('https://onchain.sv/api/v1/boostpow/rankings?app=run.relayx.io&type=order')

		return data.rankings.map(event => event).filter(event => !!event)
		
	}

	(async () => {

		var rankings = await listRankings()

		var rankingsMap = rankings.reduce((map, event) => {

			console.log("ranking", event)

			map[event.content?.paymail] = true
		...
		const events = [rankings].flat()

		for (let event of result) {

				event.bsv = event.content.satoshis / 100000000
				events.push(event)
		}
		...
		for (let event of events.reverse()) {
			...
			const html = template(Object.assign(event, {
				bsv: event.content.satoshis / 100000000
			}))
...
const handleBoost = async (txid) => {
    ...
    try {
      ...
      const result = await relayone.send({
		...
		outputs: [send, {
			to: '1Da7efs51YWHhCJdCYY2nBBz6D8LX73DUk', // midasvalley.net revenue address
			amount: 0.00065, // $0.025 
			currency: 'BSV'
		}]
	  });
